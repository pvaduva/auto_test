...
class SSHClient:
    ...
    def scp_on_source_to_localhost(self, source_file, dest_password,
        ...
        scp_cmd = ' '.join(['scp -o StrictHostKeyChecking=no -o '
                            'UserKnownHostsFile=/dev/null -r',
                            source_file, destination]).strip()
        LOG.info("Copying files from ssh client to {}: {}".format(
            to_host, scp_cmd))
        self.send(scp_cmd)
    ...
    def wait_for_cmd_output_persists(self, cmd, content, timeout=60,
        ...
        while time.time() < end_time:
            ...
            raise exceptions.SSHException(
                ...
                "seconds".format(
                    cmd, content, extra_str, time_to_stay, timeout))
...
class ContainerClient(SSHClient):
    ...
    def connect(self, retry=False, retry_interval=1, retry_timeout=60,
                prompt=None, use_current=True, use_password=False, timeout=30):
        """
            ...
            try:
                res_index = self.expect(
                    [prompt, PASSWORD_PROMPT, Prompt.ADD_HOST,
                     self.parent.get_prompt()], timeout=timeout,
                    fail_ok=False, searchwindowsize=100)
                if res_index == 3:
                    raise exceptions.SSHException(
                        "Unable to login to {}. \nOutput: "
        ...
        if user:
            ...
            for image_name in Guest.CREDS:
                ...
                LOG.warning(
                    ...
                    "in the list: {}. Use root/root to "
                    "login.".format(known_guests))
...
class ControllerClient:
    ...
    def get_active_controller(cls, name=None, fail_ok=False):
        ...
        raise exceptions.ActiveControllerUnsetException(
            ("The name - {} does not have a corresponding controller ssh "
             "session set. ssh_map: {}").format(name, cls.__lab_ssh_map))
