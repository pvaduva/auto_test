...
class ProjectsTable(tables.TableRegion):
    ...
    MODIFY_PROJECT_FORM_FIELDS = (("domain_id", "domain_name", "name", "description", "enabled"),
                                  ...
                                  ("metadata_items", "cores", "instances",
                                   ...
                                   "snapshots", "gigabytes", "ram", "security_group", "security_group_rule",
...
class ProjectsPage(basepage.BasePage):
    ...
    def manage_member_roles(self, project_name, member_name=None, roles2allocate=None, roles2deallocate=None):
        row = self._get_row_with_project_name(project_name)
        members_form = self.projects_table.manage_members(row)
    ...
    def modify_group_roles(self, project_name, group_name=None, roles2allocate=None, roles2deallocate=None):
        row = self._get_row_with_project_name(project_name)
        groups_form = self.projects_table.modify_groups(row)
