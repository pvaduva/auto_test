...
class CephStoragePoolsTable(tables.TableRegion):
    ...
    EDIT_POOL_QUOTAS_FIELDS = ("cinder_pool_gib", "glance_pool_gib", "ephemeral_pool_gib", "object_pool_gib")
...
class SystemConfigurationPage(basepage.BasePage):
    ...
    def create_address_pool(self, name, network, order=None, ranges=None, fail_ok=False):
        ...
        if not self.find_message_and_dismiss(messages.SUCCESS):
            ...
            if fail_ok and found_err:
                ...
                raise exceptions.HorizonError("No info message found after creating address pool {}".format(name))
    ...
    def delete_address_pool(self, name, fail_ok=False):
        ...
        if not self.find_message_and_dismiss(messages.SUCCESS):
            ...
            if fail_ok and found_err:
                ...
                raise exceptions.HorizonError("No info message found after deleting address pool {}".format(name))
    ...
    def update_address_pool(self, name, new_name=None, new_order=None, new_ranges=None, fail_ok=False):
        ...
        if not self.find_message_and_dismiss(messages.INFO):
            ...
            if fail_ok and found_err:
                ...
                raise exceptions.HorizonError("No info message found after updating address pool {}".format(name))
    ...
    def edit_oam(self, subnet=None, gateway=None, floating=None, controller0=None, controller1=None, cancel=False):
    ...
    def edit_filesystem(self, database=None, glance=None, backup=None,
                        scratch=None, extension=None, img_conversions=None, cancel=False):
    ...
    def check_horizon_displays(self, expt_horizon, table_name):
        ...
        for horizon_header in expt_horizon:
            if table_name == self.systems_table.name:
                horizon_value = self.get_system_info(name=expt_horizon['Name'], header=horizon_header)
            elif table_name == self.address_pools_table.name:
                horizon_value = self.get_address_pool_info(name=expt_horizon['Name'], header=horizon_header)
            elif table_name == self.ptp_table.name:
                horizon_value = self.get_ptp_info(enabled=expt_horizon['PTP Enabled'], header=horizon_header)
            elif table_name == self.oam_table.name:
                horizon_value = self.get_oam_info(subnet=expt_horizon['OAM Subnet'], header=horizon_header)
            elif table_name == self.controllerfs_table.name:
                horizon_value = self.get_controllerfs_info(storage_name=expt_horizon['Storage Name'], header=horizon_header)
            elif table_name == self.ceph_storage_pools_table.name:
                horizon_value = self.get_ceph_storage_pools_info(tier_name=expt_horizon['Ceph Storage Tier'], header=horizon_header)
