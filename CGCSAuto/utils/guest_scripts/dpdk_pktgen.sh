...
# 2vcpus setup
# ${PKTGEN} -c 0x3 -n 1 --huge-dir ${HUGETLBMNT} --proc-type primary --socket-mem 512 ${USEDEVICES} --file-prefix pg -- -p 0xFFFF -P -N -m "[1:1].0" -f /root/dpdk_pktgen.config

# 3vcpus setup
${PKTGEN} -c 0x7 -n 2 --huge-dir ${HUGETLBMNT} --proc-type primary --socket-mem 512 ${USEDEVICES} --file-prefix pg -- -p 0xFFFF -P -N -m "[1:1-2].0" -f /root/dpdk_pktgen.config
