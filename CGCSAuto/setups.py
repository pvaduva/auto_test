...
from consts.filepaths import PrivKeyPath, WRSROOT_HOME, BuildServerPath, TuxlabServerPath
...
# TODO: Setting up natbox will fail if the floating IP is down. In tc_sysinstall/conftest.py
def setup_tis_ssh(lab):
    ...
    if con_ssh is None:
        try:
            ...
            print("caught exception, trying alternate IP")
...
# currently no support for installing lab with multiple controllers with a single controller node
# Fix: overwrite the controller nodes in the lab with supplied one(s)
# Problem: It requires the user to supply each controller node if they want a certain lab
# The user should create their own install configuration file if they want to install a lab with only 1 controller
def write_installconf(lab, controller, lab_files_server, lab_files_dir, build_server, tis_build_dir, compute, storage,
                    ...
                    if str(barcode) in controller_nodes:
                        ...
                        files_dir = \
                            "{}/rt/repo/addons/wr-cgcs/layers/cgcs/extras.ND/lab/yow/{}".format(host_build_dir,
                                                                        install_helper.get_git_name(lab_dict['name']))
                        if compute_nodes:
    ...
    if lab_name and not lab_dict:
        ...
        files_dir = "{}/rt/repo/addons/wr-cgcs/layers/cgcs/extras.ND/lab/yow/{}".format(host_build_dir,
                                                                                        install_helper.get_git_name(
                                                                                            lab_dict['name']))

    # TODO: if the user tries again and ends up getting the error message in set_install_vars that would be contradictory
    if not lab_dict:
...
# TODO: make file paths: find out if TiSPath.HEAT is correct
# TODO: clean up
def setup_heat(con_ssh=None, telnet_conn=None, fail_ok=True):
    ...
    if rc != 0:
        ...
        if rc != 0:
            ...
            # TODO: the yaml files should be created after creating the resources but I think launch_stacks could be checked for earlier
            # TODO: I don't even think we need to check to see if .sh files exists if we're going to try to run them
            # we can just try to run them and return the output if the command fails
