...
def test_tc4702_swact_host(hostname, timeout, fail_ok):
    ...
    con_ssh = ControllerClient.get_active_controller()
    if fail_ok:
        code, msg = host_helper.swact_host(hostname=hostname, swact_start_timeout=timeout, swact_complete_timeout=timeout, fail_ok=fail_ok)
        if timeout == 1:
            assert code == 3
            host_helper._wait_for_swact_complete(hostname, swact_complete_timeout=90, fail_ok=False)
        else:
        ...
        if timeout == 1:
            with raises(exceptions.HostPostCheckFailed):
                host_helper.swact_host(hostname=hostname, swact_start_timeout=1, swact_complete_timeout=600, fail_ok=fail_ok)
            host_helper._wait_for_swact_complete(hostname, swact_complete_timeout=90, fail_ok=False)
        else:
            host_helper.swact_host(hostname=hostname, swact_start_timeout=timeout, swact_complete_timeout=600, fail_ok=fail_ok, con_ssh=con_ssh)
