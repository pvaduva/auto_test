...
@mark.cpe_sanity
def test_tc4697_suspend_resume_guest_instances(vm_1, vm_2):
    ...
    LOG.tc_step('Suspend the tenants')
    for vm_id in instance_list:
        ...
        instance_id = get_column_value_from_multiple_columns(instance_table,
                                                                     "Name",
                                                                      vm_name,
                                                                     'ID')
        cli.nova('suspend %s' % instance_id, auth_info=Tenant.ADMIN)

    time.sleep(30)
    LOG.tc_step('Verify that all the instances were successfully suspended')
    instance_table = table_parser.table(cli.nova('list --all', auth_info=Tenant.ADMIN))
    ...
    LOG.tc_step('Resume the tenants')
    for vm_id in instance_list:
        vm_name = nova_helper.get_vm_name_from_id(vm_id=vm_id)
        instance_id = get_column_value_from_multiple_columns(instance_table,
                                                                     "Name",
                                                                      vm_name,
                                                                     'ID')
        cli.nova('resume %s' % instance_id, auth_info=Tenant.ADMIN)

    time.sleep(30)
    LOG.tc_step('Verify that all the instances were successfully resumed')
    instance_table = table_parser.table(cli.nova('list --all', auth_info=Tenant.ADMIN))
    for vm_id in instance_list:
        vm_name = nova_helper.get_vm_name_from_id(vm_id=vm_id)
        instance_status = get_column_value_from_multiple_columns(instance_table,
