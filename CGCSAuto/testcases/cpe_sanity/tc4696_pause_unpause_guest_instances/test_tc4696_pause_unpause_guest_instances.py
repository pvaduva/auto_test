...
class LaunchInstances():
    ...
    def _pause_unpause_instance(self):
        ...
        for name in instance_list:
            instance_id = self.get_column_value_from_multiple_columns(instance_table,
                                                                         "Name",
                                                                          name,
                                                                         'ID')
            cmd = ("source /etc/nova/openrc; nova pause %s" % instance_id)  
            print ("Command executed: %s" % cmd)
            result = self.cmd_execute(cmd)

        # Verify that all the instances were successfully paused
        instance_table = self.table(self.sys_execute('/usr/bin/nova list --all'))
        ...
        for name in instance_list:
            instance_id = self.get_column_value_from_multiple_columns(instance_table,
                                                                         "Name",
                                                                          name,
                                                                         'ID')
            cmd = ("source /etc/nova/openrc; nova unpause %s" % instance_id)  
            print ("Command executed: %s" % cmd)
            result = self.cmd_execute(cmd)

        # Verify that all the instances were successfully unpaused
        instance_table = self.table(self.sys_execute('/usr/bin/nova list --all'))
...
@mark.parametrize('host_ip', [
                  ...
                  '10.10.10.2',
                  '128.224.151.212'])
def test_tc4696_pause_unpause_guest_instances(host_ip):
    launch_instances = LaunchInstances(host_ip)
    launch_instances._pause_unpause_instance()
