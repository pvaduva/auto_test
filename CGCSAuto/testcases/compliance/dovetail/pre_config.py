...
def tempest_conf_generate(computes, server_ssh):

    server_ssh.exec_sudo_cmd('sed -i "s/.*min_compute_nodes:.*/  min_compute_nodes: '+str(computes)+'/g" ${DOVETAIL_HOME}/pre_config/tempest_conf.yaml')
    server_ssh.exec_sudo_cmd('sed -i "s/.*volume_device_name:.*/  volume_device_name: vdb/g" ${DOVETAIL_HOME}/pre_config/tempest_conf.yaml')
    print(server_ssh.exec_cmd('cat ${DOVETAIL_HOME}/pre_config/tempest_conf.yaml'))
#     sample = '''compute:
...
def test():
    ...
    if len(compute_ips) == 2:
        pod_generate_2plus2('192.168.204.3', '192.168.204.4', compute_ips[0], compute_ips[1], server_ssh)

    tempest_conf_generate(len(compute_ips), server_ssh)
