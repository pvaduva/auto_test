...
def modify_hugepage(request):
    ...
    # config the page number
    # execute command
    LOG.info('Try to modify host memory after locking the host')
    a = table_parser.table( cli.system('host-memory-modify', args, auth_info=Tenant.ADMIN, fail_ok=False))
    print(a)
    # unlock the node
...
def test_huge_page_created(modify_hugepage):

    hostname = modify_hugepage['hostname']
    processor = modify_hugepage['processor']
    expected_huge_page = modify_hugepage['huge_page']
