...
def test_snapshot_large_vm_negative(add_admin_role_module, inst_backing):
    ...
    # Make first snapshot
    storage_before = float(table_parser.get_values(snaptable_, "free capacity (Gib)", **{'service': 'glance', 'backend type': 'file'})[0])
    ...
    snaptable_ = table_parser.table(cli.system("storage-usage-list", auth_info=Tenant.ADMIN))
    storage_left = float(table_parser.get_values(snaptable_, "free capacity (Gib)", **{'service': 'glance', 'backend type': 'file'})[0])
    ...
    snaptable_ = table_parser.table(cli.system("storage-usage-list", auth_info=Tenant.ADMIN))
    storage_after = float(table_parser.get_values(snaptable_, "free capacity (Gib)", **{'service': 'glance', 'backend type': 'file'})[0])
