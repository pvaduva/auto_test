...
def test_force_lock_with_mig_vms(get_hosts_with_backing):
    ...
    LOG.tc_step("Wait for 'Active' status of VMs after migration completes")
...
def test_force_lock_with_non_mig_vms(get_hosts_with_backing, add_cgcsauto_zone):
    """
    ...
    Test Teardown:
        - Remove admin role from primary tenant
        - Delete created vms
        - Remove host_under test from cgcsauto aggregate
    ...
    vm_helper.wait_for_vm_values(vm_id, fail_ok=False, **{'status': 'ACTIVE'})
    vm_helper.wait_for_vm_pingable_from_natbox(vm_id)
