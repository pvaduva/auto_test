...
def test_create_snapshot_using_boot_from_image_vm():
    ...
    * There are so images available on the system
    ...
    vm_helper.boot_vm(name=snapshot_vm, source="image", source_id=image_id,
                      cleanup='function', fail_ok=False)
...
<size>
    ...
    vol_id = cinder_helper.create_volume(source_id=image_uuid,
                                         vol_type=vol_type, size=vol_size,
                                         fail_ok=False,
                                         cleanup='function')[1]

    LOG.tc_step("Boot VM using newly created bootable volume")
    vm_id = vm_helper.boot_vm(source="volume", source_id=vol_id,
                              cleanup='function')[1]
    vm_name = vm_helper.get_vm_name_from_id(vm_id)
    snapshot_name = vm_name + "_snapshot"
    ...
    code, image_id, snapshot_id = vm_helper.create_image_from_vm(
        vm_id, image_name=snapshot_name, cleanup='function',
    ...
    snapshot_vol_id = cinder_helper.create_volume(
        name=vol_name, source_id=snapshot_id, source_type='snapshot',
        cleanup='function')[1]
