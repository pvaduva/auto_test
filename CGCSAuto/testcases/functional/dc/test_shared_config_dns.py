...
def test_dc_dns_modify(ensure_synced, scenario):
    ...
    LOG.tc_step("Ensure DNS config is not updated on unmanaged subcloud: {}".format(primary_subcloud))
    code = dc_helper.wait_for_subcloud_dns_config(subcloud=primary_subcloud, expected_dns=new_dns_servers,
...
def test_dc_dns_override_local_change(ensure_synced):
    ...
    new_dns_servers = compose_new_dns_servers(scenario='unreachable_server', prev_dns_servers=sc_dns)
    ...
    dc_helper.wait_for_sync_audit(subclouds=managed_cloud, fail_ok=True)
