...
from keywords import common, dc_helper, container_helper
...
def copy_test_apps():
    ...
    common.scp_from_test_server_to_active_controllers(
                source_path=TestServerPath.CUSTOM_APPS, dest_dir=home_dir,
                dest_name='custom_apps/', cons_ssh=cons_ssh, timeout=60,
                is_dir=True)
...
def test_launch_app_via_sysinv(copy_test_apps):
    ...
    platform_app = container_helper.get_apps(
            auth_info=central_auth, application='platform-integ-apps')
    ...
    container_helper.upload_app(
            app_name=app_name, app_version=HELM_APP_VERSION,
    ...
    LOG.tc_step("Check docker image stored in System controller \
            registry.local")
    code, output = cli.system(
            cmd="registry-image-list | fgrep hellokitty",
            ssh_client=central_ssh, fail_ok=True)
    assert code == 0
#    LOG.info("code %s, output %s", code, output)
    for subcloud in subclouds:
        ...
        LOG.info('Test platform-integ-apps is applied, on subcloud \
                {}'.format(subcloud))
        assert len(platform_app) != 0 and platform_app[0] == 'applied'
        ...
        LOG.tc_step("Check docker image stored on {} registry.central\
                ".format(subcloud))
        code, output = cli.system(
                cmd="registry-image-list | fgrep hellokitty",
                ssh_client=central_ssh, auth_info=subcloud_auth, fail_ok=True)
        assert code == 0
