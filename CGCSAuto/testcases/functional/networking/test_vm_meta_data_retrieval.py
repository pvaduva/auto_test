...
def test_vm_meta_data_retrieval():
    ...
    # retrieve meta instance id by ssh to VM from natbox and wget to
    # remote server
...
def _router_info(request):
    ...
    LOG.info("Router {} external subnet id {}".format(
        router_name, ext_gateway_subnet))
    ...
    def recover():
        ...
        if not post_router_id:
            LOG.fixture_step("Router not exist, create new router {}".format(
                router_name))
            post_router_id = network_helper.create_router(
                name=router_name)[1]
        ...
        LOG.fixture_step("Ensure all interfaces added to router {}".format(
            post_router_id))
...
def _set_external_gatewayway_info(router_id, ext_gateway_subnet, gateway_ip,
                                  is_dvr):

    LOG.info("External Gateway Subnet Id: {}".format(ext_gateway_subnet))
    LOG.info("Router {} external subnet id {}".format(router_id,
                                                      ext_gateway_subnet))
    network_helper.set_router_gateway(router_id=router_id, clear_first=False,
                                      fixed_ips=gateway_ip, enable_snat=False)
