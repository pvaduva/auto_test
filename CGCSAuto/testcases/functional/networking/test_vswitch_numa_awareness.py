...
def compare_cores_to_configure(host, func, p0, p1):
    """
    ...
    """
    proc_core_dict = host_helper.get_host_cpu_cores_for_function(host, func=func, core_type='log_core')
...
def host_to_config(request, add_admin_role_module, add_cgcsauto_zone):
    ...
    vswitch_proc_core_dict = host_helper.get_host_cpu_cores_for_function(host, func='vSwitch', core_type='log_core')
    pform_proc_core_dict = host_helper.get_host_cpu_cores_for_function(host, func='platform', core_type='log_core')

    proc1_exists = True if 1 in vswitch_proc_core_dict else False
    ...
    def revert():
        ...
        post_pform_dict = host_helper.get_host_cpu_cores_for_function(host, func='platform', core_type='log_core')
        ...
        if (vswitch_proc_core_dict != post_vswitch_dict or pform_proc_core_dict != post_pform_dict) and \
                (any([len(v) > 2 for v in post_vswitch_dict.values()]) or
    ...
    ht_enabled = host_helper.is_host_hyperthreaded(host)
    is_small_system = system_helper.is_aio_system()

    host_cpu_table = host_helper.get_host_cpu_list_table(host=host)
...
class TestVSwitchCPUReconfig:
    ...
    def test_vswitch_cpu_reconfig_positive(self, host_to_config, flavor_, platform, vswitch, ht_required, cpe_required):
        ...
        assert host == vm_helper.get_vm_host(vm_id), "VM is not booted on configured host"
...
class TestNovaSchedulerAVS:
    ...
    @fixture(scope='class')
    def hosts_configured(self, check_avs_pattern, get_hosts, config_host_class):
        ...
        def _mod_host(host__, p0, p1):
            host_helper.modify_host_cpu(host__, func, p0=p0, p1=p1)

        hosts_to_config = {}

        h0p0_is_match, h0p0_p0_to_conf, h0p0_p1_to_conf = compare_cores_to_configure(host0, func, p0=2, p1=0)
        if h0p0_is_match:
            p0_host = host0  # No config for host0
            p1_host = host1
            h1p1_is_match, h1p1_p0_to_conf, h1p1_p1_to_conf = compare_cores_to_configure(host1, func, p0=0, p1=2)
            if not h1p1_is_match:  # config host1
                hosts_to_config[p1_host] = {'p0': h1p1_p0_to_conf, 'p1': h1p1_p1_to_conf}

        else:
            h1p0_is_match, h1p0_p0_to_conf, h1p0_p1_to_conf = compare_cores_to_configure(host1, func, p0=2, p1=0)
            if h1p0_is_match:
                p0_host = host1
                p1_host = host0
                h0p1_is_match, h0p1_p0_to_conf, h0p1_p1_to_conf = compare_cores_to_configure(host0, func, p0=0,
                                                                                             p1=2)
                if not h0p1_is_match:
                    hosts_to_config[p1_host] = {'p0': h0p1_p0_to_conf, 'p1': h0p1_p1_to_conf}
            else:
                h1p1_is_match, h1p1_p0_to_conf, h1p1_p1_to_conf = compare_cores_to_configure(host1, func, p0=0,
                                                                                             p1=2)
                if h1p1_is_match:
                    p1_host = host1
                    p0_host = host0
        ...
        is_match_p0 = compare_cores_to_configure(p0_host, func, p0=2, p1=0)[0]
        is_match_p1 = compare_cores_to_configure(p1_host, func, p0=0, p1=2)[0]
    ...
    def _test_vswitch_numa_affinity_boot_vm(self, hosts_configured, vswitch_numa_affinity, numa_0,
        ...
        code, vm_id, err = vm_helper.boot_vm('numa_affinity', flavor=flv_id, cleanup='function',
        ...
        if expt_err:
            ...
            actual_err = vm_helper.get_vm_fault_message(vm_id)
    ...
    def _test_vswitch_numa_affinity_sched_vms_two_hosts_avail(self, cal_vm_cores_two_hosts, vswitch_numa_affinity):
        ...
        for i in range(vms_num):
            code, vm_id, err = vm_helper.boot_vm('vswitch_numa', flavor=flv_id, cleanup='function',
        ...
        if vswitch_numa_affinity is not None:
            ...
            vms_to_del = vm_helper.get_vms_on_host(other_host)
    ...
    def _test_vswitch_numa_affinity_sched_vms_one_host_avail(self, cal_vm_cores_one_host):
        ...
        for i in range(vm_count):
            code, vm_id, err = vm_helper.boot_vm('vswitch_numa_one_host', flavor=flv_id, cleanup='function',
                                                 avail_zone='cgcsauto', fail_ok=True)
            assert 0 == code, "VM is not booted successfully. Details: {}".format(err)
            vm_host, vm_numa = vm_helper.get_vm_host_and_numa_nodes(vm_id)
            vm_numa = vm_numa[0]
    ...
    def get_target_host_and_flavors(self, hosts_configured, request):
        ...
        ht_enabled = host_helper.is_host_hyperthreaded(target_host)
        ...
        vswitch_cores_dict = host_helper.get_host_cpu_cores_for_function(target_host, func='vSwitch')
    ...
    @mark.parametrize(('vswitch_numa_affinity', 'resize_revert'), [
        param('strict', 'confirm', marks=mark.p3),
        param('prefer', 'confirm', marks=mark.p3),
        param('prefer', 'revert', marks=mark.p3)
    ])
...
class TestSpanNumaNodes:
    @fixture(scope='class')
    def span_numa_hosts(self, check_avs_pattern, get_hosts, config_host_class):
        host0, host1, storage_backing, ht_enabled = get_hosts
        ...
        ht_enabled = host_helper.is_host_hyperthreaded(host_span)
    ...
    def _test_vm_actions_vswitch_span_numa_nodes(self, span_numa_hosts, vcpus, vswitch_affinity, numa0, numa0_cpus,
        ...
        code, vm_id, err = vm_helper.boot_vm('span_numa_{}'.format(vswitch_affinity), cleanup='function',
...
def test_compute_cpu_kernel_boot_args(hosts=None):
    ...
    for host in hosts:
        is_lowlat = system_helper.is_lowlatency_host(host=host)
        funcs = ('Platform', 'vSwitch', None)
        platform_cores, vswitch_cores, all_cores = \
            host_helper.get_host_cpu_cores_for_function(hostname=host, func=funcs, thread=None, rtn_dict_per_proc=False)

        rcu_nocbs = list(set(all_cores) - set(platform_cores))
        expt_dict = {
            'rcu_nocbs': rcu_nocbs,
            'kthread_cpus': platform_cores,
            'irqaffinity': platform_cores,
            'isolcpus': rcu_nocbs if is_lowlat else vswitch_cores
        }
