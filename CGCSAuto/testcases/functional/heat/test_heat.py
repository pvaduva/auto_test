...
def verify_heat_resource(to_verify=None,template_name=None,stack_name=None,auth_info=None):
    ...
    if to_verify is 'volume' :
        ...
        vm_id = nova_helper.get_vm_id_from_name(vm_name=vm_name)
        if vm_id:
            return 0
...
def verify_basic_template(template_name=None, con_ssh=None, auth_info=None):
    """
    ...
    for item in to_verify:
        ...
        verify_result = verify_heat_resource(to_verify=item, template_name=t_name,stack_name=stack_name,auth_info=auth_info)
