...
def test_force_reboot_host(host_type):
    ...
    hosts = system_helper.get_hosts(availability=(HostAvailState.AVAILABLE, HostAvailState.DEGRADED),
                                    personality=host_type)
    if not hosts:
        skip("No available or degraded {} host found on system".format(host_type))
