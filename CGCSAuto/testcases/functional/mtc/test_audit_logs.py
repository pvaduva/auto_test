...
def test_sudo_log():
    """
    TC5089 Test the logs created during successful and failed sudo commands
    ...
    for line in out:
        for i in range(0, len(searching_for)):
            add = True
            for part in searching_for[i]:
                if part not in line:
                    add = False
                    break
            if add and searching_for[i] not in found:
                found.append(searching_for[i])
                LOG.tc_step("Found {}".format(line))
                break
...
def test_postgress():
    """
    TC5092 Test postgres login and logout logs

    Test Steps:
        - Check logs for log entries about postgres login/logout
        - If there were none found, wait 30 seconds for them to be generated
        - Check for the logs again
...
def test_sudo_su():
    """
    TC5093 Test logs created by sudo su
    ...
    exec_sudo_su_fail(ssh)
    code, out = ssh.exec_cmd("tail /var/log/auth.log", fail_ok=True)
    out = out.split('\n')
    ssh.close()
    searching_for = [['sudo: notice pam_unix(sudo:auth): authentication failure; logname=wrsroot ',
                      'ruser=wrsroot rhost=  user=wrsroot'],
                     ['sudo: alert  wrsroot : 1 incorrect password attempt',
                      'PWD=/home/wrsroot ; USER=root ; COMMAND=/usr/bin/su']]
    found = []
