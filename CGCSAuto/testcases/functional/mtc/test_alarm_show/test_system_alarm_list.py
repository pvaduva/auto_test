...
def check_alarms_raised_destroyed_after_action(inst_id, action,
        table_nova=None, nova_header_key=None, nova_val=None,
        search_key_nova=None, table_alarm=None, alarm_header_key=None,
        alarm_val=None, search_key_alarm=None, destroy=False):
    """
    Method to check alarms raised/destroyed after action with VM
    """
    if action:
        instance_helpers.execute_action(
            actions_list=[action], instance_name=inst_id)

    LOG.info("Verify that state changed.")
    cli_helpers.check_table_info("list", table_nova, nova_header_key,
                                 nova_val,
                                 search_key=search_key_nova)
    LOG.info("Wait 1 minute and verify that alarm is raised/destroyed")
    for i, search_key_alarm in enumerate(search_key_alarm):
        LOG.info("Check that value %s is in column %s" %
                 (alarm_val[i], alarm_header_key[i]))
        alarm_uuid = cli_helpers.wait_until_entry_in_table(
            alarm_header_key[i], alarm_val[i], "UUID", 120, destroy)
        if not destroy:
            LOG.info("Check that values %s, %s are in column with row %s" %
                     (search_key_alarm, table_alarm[i], alarm_uuid))
            cli_helpers.check_table_info(
                "alarm-list", table_alarm[i], match_header_key="UUID",
                match_val=alarm_uuid, search_key=search_key_alarm)
