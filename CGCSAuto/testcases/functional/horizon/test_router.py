...
def test_router_gateway(routers_pg_action):
    ...
    routers_pg_action.clear_gateway(ROUTER_NAME)
    ...
    assert routers_pg_action.get_router_info(ROUTER_NAME, 'External Network') == '-'

    LOG.tc_step('Set new gateway to the router')
    routers_pg_action.set_gateway(ROUTER_NAME, external_network='external-net0')
    assert routers_pg_action.find_message_and_dismiss(messages.SUCCESS)
    assert not routers_pg_action.find_message_and_dismiss(messages.ERROR)

    LOG.tc_step('Verify the new set gateway appears in the gateway table')
    assert routers_pg_action.get_router_info(ROUTER_NAME, 'External Network') == 'external-net0'
    horizon.test_result = True
...
def test_router_add_delete_interface(routers_pg_action):
    ...
    router_interfaces_page = routers_pg_action.go_to_interfaces_page(ROUTER_NAME)
...
def test_router_overview_data(routers_pg_action):
    ...
    router_overview_page = routers_pg_action.go_to_overview_page(ROUTER_NAME)
...
def test_router_admin_edit(routers_pg_action):
    ...
    admin_routers_page = admin_routerspage.RoutersPage(routers_pg_action.driver, port=routers_pg_action.port)
