...
class TestRouters(helper.AdminTestCase):
    ROUTER_NAME = None

    @fixture(scope='function')
    def routers_pg(self, home_pg, request):
        LOG.fixture_step('Go to Project > Network > Routers')
        self.ROUTER_NAME = helper.gen_resource_name('router')
        routers_pg = project_routerspage.RoutersPage(home_pg.driver)
    ...
    def routers_pg_action(self, routers_pg, request):
        ...
        self._create_router(routers_pg)

        def teardown():
            LOG.fixture_step('Delete router {}'.format(self.ROUTER_NAME))
            self._delete_router(routers_pg)
    ...
    def _delete_router(self, routerspage):
        ...
        assert routerspage.find_message_and_dismiss(messages.SUCCESS)
        assert not routerspage.find_message_and_dismiss(messages.ERROR)
        assert not routerspage.is_router_present(self.ROUTER_NAME)
    ...
    def _create_interface(self, interfaces_page):
        ...
        assert interfaces_page.find_message_and_dismiss(messages.SUCCESS)
        assert not interfaces_page.find_message_and_dismiss(messages.ERROR)
        assert interfaces_page.is_interface_present(interface_name)
        assert interfaces_page.is_interface_status(interface_name, 'Down')

    def _delete_interface(self, interfaces_page, interface_name):
        interfaces_page.delete_interface(interface_name)
        assert interfaces_page.find_message_and_dismiss(messages.SUCCESS)
        assert not interfaces_page.find_message_and_dismiss(messages.ERROR)
        assert not interfaces_page.is_interface_present(interface_name)
    ...
    def test_router_admin_edit(self, routers_pg):
        """
        ...
        LOG.tc_step('Create new router {}'.format(self.ROUTER_NAME))
        self._create_router(routers_pg)
        ...
        assert admin_routers_page.find_message_and_dismiss(messages.SUCCESS)
        assert not admin_routers_page.find_message_and_dismiss(messages.ERROR)
        assert not admin_routers_page.is_router_present(new_name)
