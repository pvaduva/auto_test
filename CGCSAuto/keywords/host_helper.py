            ...
            if hosts_avail and (check_hypervisor_up or check_webservice_up):
                ...
                if check_hypervisor_up and computes:
                    ...
                    res, hosts_hypervisordown = wait_for_hypervisors_up(
                        ...
                        timeout=HostTimeout.HYPERVISOR_UP, auth_info=nova_auth)
...
def wait_for_task_clear_and_subfunction_ready(
        hosts, fail_ok=False, con_ssh=None, use_telnet=False,
        con_telnet=None, timeout=HostTimeout.SUBFUNC_READY,
        auth_info=Tenant.get('admin_platform')):
    if isinstance(hosts, str):
        hosts = [hosts]
    ...
    LOG.info("Waiting for task clear and subfunctions enable/available "
             "(if applicable) for hosts: {}".
             format(hosts_to_check))
    end_time = time.time() + timeout
    while time.time() < end_time:
        hosts_vals = system_helper.get_hosts_values(
            hosts_to_check, ['subfunction_avail', 'subfunction_oper', 'task'],
        ...
        if not hosts_to_check:
            LOG.info("Hosts task cleared and subfunctions (if applicable) are "
                     "now in enabled/available states")
            return True
...
def lock_host(host, force=False, lock_timeout=HostTimeout.LOCK,
              ...
              use_telnet=False, con_telnet=None, fail_ok=False,
              check_first=True, swact=False,
    ...
    """
    host_avail, host_admin = system_helper.get_host_values(
        host, ('availability', 'administrative'), con_ssh=con_ssh,
    ...
    is_aio_dup = system_helper.is_aio_duplex(
        con_ssh=con_ssh, use_telnet=use_telnet, con_telnet=con_telnet,
        auth_info=auth_info)

    if swact:
        if system_helper.is_active_controller(
                host, con_ssh=con_ssh, use_telnet=use_telnet,
                con_telnet=con_telnet, auth_info=auth_info) and \
            ...
            LOG.info("{} is active controller, swact first before attempt to "
                     "lock.".format(host))
            swact_host(host, con_ssh=con_ssh, use_telnet=use_telnet,
                       con_telnet=con_telnet, auth_info=auth_info)
            if is_aio_dup:
                time.sleep(90)

    if check_cpe_alarm and is_aio_dup:
        LOG.info("For AIO-duplex, wait for cpu usage high alarm gone on "
                 "active controller before locking standby")
        ...
        system_helper.wait_for_alarms_gone(
            ...
            fail_ok=fail_ok, con_ssh=con_ssh, timeout=300,
    ...
    exitcode, output = cli.system('host-lock', positional_arg,
                                  ssh_client=con_ssh, use_telnet=use_telnet,
                                  con_telnet=con_telnet, fail_ok=fail_ok,
                                  auth_info=auth_info)
    ...
    if admin_val != HostAdminState.LOCKED:
        if 'Locking' not in task_val:
            system_helper.wait_for_host_values(
                host=host, timeout=30, check_interval=0, fail_ok=True,
                task='Locking', con_ssh=con_ssh, use_telnet=use_telnet,
                con_telnet=con_telnet, auth_info=auth_info)

        # Wait for task complete. If task stucks, fail the test regardless.
        # Perhaps timeout needs to be increased.
        system_helper.wait_for_host_values(
            host=host, timeout=lock_timeout, task='', fail_ok=False,
            con_ssh=con_ssh, use_telnet=use_telnet, con_telnet=con_telnet,
            auth_info=auth_info)

        if not system_helper.wait_for_host_values(
                host, timeout=20, administrative=HostAdminState.LOCKED,
                con_ssh=con_ssh, use_telnet=use_telnet,
                con_telnet=con_telnet, auth_info=auth_info):
    ...
    if system_helper.wait_for_host_values(host, timeout=timeout,
        ...
        if system_helper.wait_for_host_values(
                host, timeout=timeout, availability=HostAvailState.ONLINE,
                auth_info=auth_info, con_ssh=con_ssh, use_telnet=use_telnet,
                con_telnet=con_telnet):
            if system_helper.wait_for_host_values(
                    host, timeout=HostTimeout.TASK_CLEAR, task='',
                    auth_info=auth_info, con_ssh=con_ssh, use_telnet=use_telnet,
                    con_telnet=con_telnet):
...
def _wait_for_simplex_reconnect(con_ssh=None,
    ...
    if not use_telnet:
        ...
        if not con_telnet:
            raise ValueError("con_telnet has to be provided when "
                             "use_telnet=True.")
    ...
    if not duplex_direct:
        ...
        _wait_for_openstack_cli_enable(con_ssh=con_ssh, use_telnet=use_telnet,
                                       con_telnet=con_telnet,
                                       auth_info=auth_info, fail_ok=False,
                                       timeout=timeout, check_interval=10,
...
def unlock_host(host, timeout=HostTimeout.CONTROLLER_UNLOCK,
    ...
    if check_first:
        if system_helper.get_host_values(host, 'availability', con_ssh=con_ssh,
                                         use_telnet=use_telnet,
                                         auth_info=auth_info,
                                         con_telnet=con_telnet, )[0] in \
                [HostAvailState.OFFLINE, HostAvailState.FAILED]:
            LOG.info("Host is offline or failed, waiting for it to go online, "
        ...
        if system_helper.get_host_values(
                host, 'administrative', con_ssh=con_ssh,
                use_telnet=use_telnet, auth_info=auth_info,
                con_telnet=con_telnet)[0] == HostAdminState.UNLOCKED:
    ...
    if not system_helper.wait_for_host_values(
            host, timeout=60, administrative=HostAdminState.UNLOCKED,
            con_ssh=con_ssh, use_telnet=use_telnet, con_telnet=con_telnet,
            fail_ok=fail_ok, auth_info=auth_info):
        return 2, "Host is not in unlocked state"

    if not system_helper.wait_for_host_values(
            host, timeout=timeout, fail_ok=fail_ok, check_interval=10,
            con_ssh=con_ssh, use_telnet=use_telnet, con_telnet=con_telnet,
            auth_info=auth_info, availability=[HostAvailState.AVAILABLE,
    ...
    if check_hypervisor_up or check_webservice_up or check_subfunc:
        ...
        subfunc, personality = system_helper.get_host_values(
            host, fields=('subfunctions', 'personality'), con_ssh=con_ssh,
            auth_info=auth_info, use_telnet=use_telnet, con_telnet=con_telnet)
        ...
        if check_hypervisor_up and is_compute:
            if container_helper.is_stx_openstack_deployed(
                ...
                if not wait_for_hypervisors_up(
                        host, fail_ok=fail_ok, con_ssh=con_ssh,
                        auth_info=nova_auth, use_telnet=use_telnet,
                        con_telnet=con_telnet,
    ...
    if system_helper.get_host_values(
            host, 'availability', con_ssh=con_ssh, auth_info=auth_info,
...
def unlock_hosts(hosts, timeout=HostTimeout.CONTROLLER_UNLOCK, fail_ok=True,
    ...
    for host in hosts:
        if system_helper.get_host_values(
                host, 'administrative', con_ssh=con_ssh, auth_info=auth_info,
                use_telnet=use_telnet, con_telnet=con_telnet)[0] == \
                HostAdminState.UNLOCKED:
            message = "Host already unlocked. Do nothing"
    ...
    if not system_helper.wait_for_hosts_states(
            hosts_to_check, timeout=60, administrative=HostAdminState.UNLOCKED,
            con_ssh=con_ssh, use_telnet=use_telnet, con_telnet=con_telnet,
            auth_info=auth_info):
        LOG.warning("Some host(s) not in unlocked states after 60 seconds.")

    if not system_helper.wait_for_hosts_states(
            hosts_to_check, timeout=timeout, check_interval=10, con_ssh=con_ssh,
            use_telnet=use_telnet, con_telnet=con_telnet, auth_info=auth_info,
            availability=[HostAvailState.AVAILABLE, HostAvailState.DEGRADED]):
        LOG.warning("Some host(s) state did not change to available or "
                    "degraded  within timeout")
    ...
    hosts_other = \
        list(set(hosts_unlocked) - set(hosts_avail) - set(hosts_degrd))
    ...
    if hosts_avail and (check_hypervisor_up or check_webservice_up):
        ...
        all_nodes = system_helper.get_hosts_per_personality(
            con_ssh=con_ssh, use_telnet=use_telnet, auth_info=auth_info,
            con_telnet=con_telnet)
        computes = list(set(hosts_avail) & set(all_nodes['compute']))
    ...
    if check_nodes_ready and (hosts_avail or hosts_degrd) and not use_telnet:
        ...
        res_nodes, hosts_not_ready = kube_helper.wait_for_nodes_ready(
            hosts=hosts_to_wait, timeout=180, con_ssh=con_ssh, fail_ok=fail_ok)
        if hosts_not_ready:
        ...
        if check_containers:
            ...
            if not (res_app and res_pods):
                ...
                hosts_to_update = \
                    list((set(hosts_to_wait) - set(hosts_not_ready)))
                hosts_avail = []
    ...
    if not len(res) == len(hosts):
        raise exceptions.CommonError("Something wrong with the keyword. "
                                     "Number of hosts in result is incorrect.")
...
def _wait_for_openstack_cli_enable(con_ssh=None, timeout=HostTimeout.SWACT,
                                   ...
                                   reconnect=True, use_telnet=False,
                                   con_telnet=None, single_node=None,
                                   auth_info=Tenant.get('admin_platform')):
    """
    ...
    def check_sysinv_cli():
        ...
        cli.system('show', ssh_client=con_ssh, use_telnet=use_telnet,
                   con_telnet=con_telnet, auth_info=auth_info,
                   timeout=10)
        time.sleep(10)
        active_con = system_helper.get_active_controller_name(
        ...
        if ((single_node or (
            ...
            LOG.info("Simplex system in locked state. Wait for task to "
                     "clear only")
    ...
    def check_nova_cli():
        ...
        cli.openstack('server list', ssh_client=con_ssh, auth_info=nova_auth,
                      timeout=10, use_telnet=use_telnet,
                      con_telnet=con_ssh)
        LOG.info("nova cli enabled")

    cli_enable_end_time = time.time() + timeout
    LOG.info("Waiting for system cli and subfunctions to be ready and nova cli "
             "(if stx-openstack applied) to be "
    ...
    while time.time() < cli_enable_end_time:
        try:
            ...
            if not use_telnet and not con_ssh.is_connected():
                if reconnect:
                    LOG.info("con_ssh connection lost while waiting for system "
                             "to recover. Attempt to reconnect...")
...
def swact_host(hostname=None, swact_start_timeout=HostTimeout.SWACT,
    ...
    if hostname != active_host:
        system_helper.wait_for_host_values(hostname,
                                           ...
                                           auth_info=auth_info,
                                           use_telnet=use_telnet,
                                           con_telnet=con_telnet, task='')
        return 2, "{} is not active controller host, thus swact request " \
                  "failed as expected.".format(hostname)

    if use_telnet:
        rtn = wait_for_swact_complete_tel_session(
            hostname, swact_start_timeout=swact_start_timeout,
    ...
    if rtn[0] == 0:
        ...
        try:
            if use_telnet:
                new_active_host = 'controller-1' if \
                    hostname == 'controller-0' else 'controller-0'
                telnet_session = get_host_telnet_session(new_active_host)
                res = wait_for_webservice_up(new_active_host, use_telnet=True,
                ...
                res = wait_for_webservice_up(
                    ...
                    fail_ok=fail_ok,  auth_info=auth_info, con_ssh=con_ssh)[0]
                if not res:
                    return 5, "Web-services for new controller is not active"
                ...
                if system_helper.is_aio_duplex(con_ssh=con_ssh,
                    ...
                    for host in ('controller-0', 'controller-1'):
                        ...
                        if not task_aff_res:
                            msg = "tasks affining incomplete on {} after " \
                                  "swact from {}".format(host, hostname)
...
def wait_for_swact_complete_tel_session(
        before_host, swact_start_timeout=HostTimeout.SWACT,
    ...
    Returns (tuple):
        ...
        # returns when fail_ok=True
        (4, "Swact did not start within <swact_start_timeout>")
        (5, "Active controller did not change after swact within
            <swact_complete_timeou>")
    ...
    new_active_controller = 'controller-1' if before_host == 'controller-0' \
        else 'controller-0'
    host_telnet_session = get_host_telnet_session(new_active_controller)

    if host_telnet_session is None:
        err_msg = "Cannot open telnet session with new active controller {}".\
            format(new_active_controller)
        if fail_ok:
            return 3, err_msg
        else:
            raise exceptions.HostPostCheckFailed(err_msg)
    ...
    swacted = False
    while not swacted:
        host_telnet_session.send("source /etc/platform/openrc")
        index, match, output = host_telnet_session.expect(
            [bytes(Prompt.ADMIN_PROMPT, 'utf-8')], timeout=2)
        if match:
            swacted = True
        if time.time() > end_swact_start:
            if fail_ok:
                return 4, "Swact did not start within {}".format(
                    swact_start_timeout)
    ...
    # Give it sometime before openstack cmds enables on after host
    _wait_for_openstack_cli_enable(use_telnet=True,
                                   con_telnet=host_telnet_session)

    after_host = system_helper.get_active_controller_name(
        use_telnet=True, con_telnet=host_telnet_session)
    LOG.info("Host before swacting: {}, host after swacting: {}".
             format(before_host, after_host))

    if before_host == after_host:
        if fail_ok:
            return 5, "Active controller did not change after swact within {}".\
                format(swact_complete_timeout)
        raise exceptions.HostPostCheckFailed(
            "Swact failed. Active controller host did not change")
    ...
    drbd_res = system_helper.wait_for_alarm_gone(
        alarm_id=EventLogID.CON_DRBD_SYNC, entity_id=after_host,
        use_telnet=True, con_telnet=host_telnet_session, strict=False,
...
def wait_for_hypervisors_up(hosts, timeout=HostTimeout.HYPERVISOR_UP,
    ...
    hypervisors = get_hypervisors(con_ssh=con_ssh, use_telnet=use_telnet,
                                  con_telnet=con_telnet, auth_info=auth_info)
...
def wait_for_webservice_up(hosts, timeout=HostTimeout.WEB_SERVICE_UP,
    ...
    hosts_to_check = list(hosts)
    LOG.info("Waiting for {} to be active for web-service in system "
             "servicegroup-list...".format(hosts_to_check))
    end_time = time.time() + timeout

    while time.time() < end_time:
        # need to check for strict True because 'go-active' state is not
        # 'active' state
        active_hosts = system_helper.get_servicegroups(
            fields='hostname', service_group_name='web-services',
...
def modify_host_interface(host, interface, pnet=None, ae_mode=None,
    ...
    if network:
        assign_interface_network(host, interface=interface, network=network,
                                 con_ssh=con_ssh, auth_info=auth_info)
...
def compare_host_to_cpuprofile(host, profile_uuid):
    ...
    platform_cores = table_parser.get_value_two_col_table(
        table_, field='platform cores')
    vswitch_cores = table_parser.get_value_two_col_table(
        table_, field='vswitch cores')
    shared_cores = table_parser.get_value_two_col_table(
        table_, field='shared cores')
    vm_cores = table_parser.get_value_two_col_table(
        table_, field='vm cores')
...
def get_vswitch_port_engine_map(host_ssh):
    ...
    Notes: assume the output format will be: 'port-map="0:1,2 1:12,13"',
        'port-map="0,1:1,2"', or 'port-map="0:1,2"'

    Returns (dict): format: {<proc_id> (str): <log_cores> (list), ...}
        e.g., {'0': ['1', '2'], '1': ['1', '2']}

    """
    output = host_ssh.exec_cmd(
        'grep --color="never" "^port-map=" /etc/vswitch/vswitch.ini',
        fail_ok=False)[1]
    ...
    LOG.info("ports/cores mapping on {} is: {}".format(host_ssh.get_hostname(),
                                                       host_vswitch_dict))
...
def get_vcpus_info_in_log(host_ssh, numa_nodes=None, rtn_list=False):
    ...
    Returns (dict|list):
        Examples: { 0: {'pinned_cpulist': [], 'unpinned_cpulist': [3, 4, 5,...],
                    'cpu_usage': 0.0, 'pinned': 0, ...},
                    1: {....}}
    ...
    for numa_node in numa_nodes:
        ...
        # sample output:
        # 2016-07-15 16:20:50.302 99972 INFO nova.compute.resource_tracker
        # [req-649d9338-ee0b-477c-8848-
        # 89cc94114b58 - - - - -] Numa node=1; cpu_usage:32.000,
        # pcpus:36, pinned:32, shared:0.000, unpinned:4;
        # pinned_cpulist:18-19,21-26,28-35,54-55,57-62,64-71,
        # unpinned_cpulist:20,27,56,63
        output = host_ssh.exec_cmd('cat /var/log/nova/nova-compute.log | '
                                   'grep -i -E "Numa node={}; .*unpinned:" '
                                   '| tail -n 1'.format(numa_node),
                                   fail_ok=False)[1]
        if not output:
        ...
        cpu_info = output.split(sep="Numa node={}; ".format(numa_node))[-1].\
            replace('; ', ', '). split(sep=', ')
    ...
    LOG.info("VCPU info for {} parsed from compute-nova.log: {}".
             format(host_ssh.get_hostname(), res_dict))
    if rtn_list:
...
def get_vcpus_for_instance_via_virsh(host_ssh, instance_name, rtn_list=False):
    """
    Get a list of pinned vcpus for given instance via 'sudo virsh vcpupin
    <instance_name>'
    ...
    Returns (list|dict): list of vcpus ids used by specified instance such as
    [8, 9], or {0: [8], 1: [9]}

    """

    output = host_ssh.exec_sudo_cmd("virsh vcpupin {} | grep -v '^[ \t]*$'".
                                    format(instance_name), fail_ok=False)[1]
...
def ssh_to_build_server(bld_srv=None, user=TestFileServer.USER,
    ...
    Args:
        bld_srv (str|dict): build server ip, name or dictionary (choose from
            consts.build_serve.BUILD_SERVERS)
    ...
    if isinstance(bld_srv, str):
        for bs in BUILD_SERVERS:
            ...
            raise exceptions.BuildServerError(
                "Requested build server - {} is not found. Choose server ip or "
                "server name from: {}".format(bld_srv, BUILD_SERVERS))
    elif bld_srv not in BUILD_SERVERS:
        raise exceptions.BuildServerError(
            "Unknown build server: {}. Choose from: {}".format(
                bld_srv, BUILD_SERVERS))

    prompt = prompt if prompt else Prompt.BUILD_SERVER_PROMPT_BASE.format(
        user, bld_srv['name'])
...
def get_host_telnet_session(host, login=True, lab=None):
    ...
    if host_node is None:
        raise ValueError("A node object for host {} is not defined in lab "
                         "dict : {}".format(host, lab))
    ...
    host_node.telnet_conn = telnetlib.TelnetClient(
        host=host_node.telnet_ip,
        port=int(host_node.telnet_port),
        hostname=host_node.name,
        negotiate=host_node.telnet_negotiate,
        vt100query=host_node.telnet_vt100query,
        console_log_file=log_dir + "/" + host_node.name + ".telnet.log")
...
def power_on_host(host, fail_ok=False, timeout=HostTimeout.REBOOT,
                  unlock=True, con_ssh=None):
    """
    ...
    """
    admin_state, avail_state = system_helper.get_host_values(
        host=host, fields=['administrative', 'availability'], con_ssh=con_ssh)
    if HostAvailState.POWER_OFF != avail_state or HostAdminState.LOCKED != \
            admin_state:
        LOG.warning("Attempt to power-on {} while it's in {} & {} states".
                    format(host, admin_state, avail_state))

    code, output = cli.system('host-power-on', host, ssh_client=con_ssh,
                              fail_ok=fail_ok)
    if code != 0:
        return 1, output

    system_helper.wait_for_host_values(host=host, timeout=60,
                                       task=HostTask.POWERING_ON,
                                       con_ssh=con_ssh, fail_ok=True)
    system_helper.wait_for_host_values(host=host, timeout=timeout,
                                       availability=HostAvailState.ONLINE,
                                       fail_ok=False)
...
def get_host_port_pci_address_for_net_type(
        host, net_type='mgmt', rtn_list=True, con_ssh=None,
        auth_info=Tenant.get('admin_platform')):
    """
