...
def get_sys_type(con_ssh=None, use_telnet=False, con_telnet=None):
    ...
    if is_aio:
        ...
        if len(get_controllers(con_ssh=con_ssh,
                               use_telnet=use_telnet, con_telnet=con_telnet,
...
def is_aio_duplex(con_ssh=None, use_telnet=False, con_telnet=None,
                  auth_info=Tenant.get('admin_platform')):
    """
...
def is_aio_system(controller_ssh=None, controller='controller-0',
    ...
    subfunc = get_host_values(host=controller, fields='subfunctions',
                              con_ssh=controller_ssh, auth_info=auth_info,
...
def get_controllers(administrative=None, operational=None, availability=None,
    ...
    """
    return get_hosts(personality='controller',
...
def get_computes(administrative=None, operational=None,
...
def get_hosts(personality=None, administrative=None, operational=None,
              ...
              exclude=False, con_ssh=None, use_telnet=False, con_telnet=None,
              auth_info=Tenant.get('admin_platform'),
    ...
    Args:
        ...
        rtn_dict (bool): Whether to return dict where each field is a key, and
        value is a list
    ...
    table_ = table_parser.table(
        cli.system('host-list', ssh_client=con_ssh, use_telnet=use_telnet,
                   con_telnet=con_telnet,
                   auth_info=auth_info)[1])
...
def get_hosts_per_personality(availability=None, administrative=None,
    ...
    Args:
        ...
        use_telnet
        con_telnet
        rtn_tuple (bool): whether to return tuple instead of dict.
            i.e., <controllers>, <computes>, <storages>

    Returns (dict|tuple):
    e.g., {'controller': ['controller-0', 'controller-1'],
        'compute': ['compute-0', 'compute-1], 'storage': []}
    ...
    for personality in personalities:
        personality_tmp = 'compute|worker' if personality == 'compute' \
            else personality
...
def get_active_controller_name(con_ssh=None, use_telnet=False, con_telnet=None,
    ...
    """
    return get_active_standby_controllers(con_ssh=con_ssh,
                                          con_telnet=con_telnet,
                                          use_telnet=use_telnet,
...
def get_active_standby_controllers(con_ssh=None, use_telnet=False,
                                   con_telnet=None,
                                   auth_info=Tenant.get('admin_platform')):
    """
    ...
    active_con = table_parser.get_values(table_, 'hostname',
                                         state='active', strict=False)[0]
    standby_con = table_parser.get_values(table_, 'hostname',
                                          state='standby', strict=False)
...
def get_alarms_table(uuid=True, show_suppress=False, query_key=None,
    ...
    for i in range(retry + 1):
        ...
        raise exceptions.CLIRejected('fm alarm-list cli got rejected after {} '
                                     'retries: {}'.format(retry, output))
...
def get_alarms(fields=('Alarm ID', 'Entity ID'), alarm_id=None,
    ...
    Args:
        ...
        reason_text (str): reason text to filter out the table
            (strict defined in param)
        entity_id (str): entity instance id to filter out the table
            (strict defined in param)
        ...
        query_value (str): value in --query <key>=<value> passed to
            fm alarm-list
        query_type (str): 'string', 'integer', 'float', or 'boolean'
        mgmt_affecting (bool)
        con_ssh (SSHClient):
        auth_info (dict):
        combine_entries (bool): return list of strings when set to True, else
...
def get_events_table(limit=5, show_uuid=False, show_only=None,
    ...
    table_ = table_parser.table(
        cli.fm('event-list ', args, ssh_client=con_ssh,
               use_telnet=use_telnet, con_telnet=con_telnet,
               auth_info=auth_info)[1])
...
def _compose_events_table(output, uuid=False):
    if not output['headers']:
        headers = ['UUID', 'Time Stamp', 'State', 'Event Log ID',
...
def __process_query_args(args, query_key, query_value, query_type):
    if query_key:
        if not query_value:
            raise ValueError("Query value is not supplied for key - {}".
                             format(query_key))
...
def wait_for_events(timeout=60, num=30, uuid=False, show_only=None,
                    ...
                    regex=False, strict=True, check_interval=5,
...
def delete_alarms(alarms=None, fail_ok=False, con_ssh=None,
    ...
    Args:
        ...
        fail_ok (bool): whether or not to raise exception if any alarm
            failed to delete
        con_ssh (SSHClient):
        auth_info (dict):
        use_telnet
    ...
    """
    if alarms is None:
        alarms_tab = get_alarms_table(uuid=True, use_telnet=use_telnet,
                                      con_telnet=con_telnet)
        alarms = []
    ...
    if undeleted_alarms:
        ...
        LOG.warning(
            ...
            "exists.\nAlarm delete results: {}".format(res))
...
def wait_for_alarm_gone(alarm_id, entity_id=None, reason_text=None,
    ...
    while time.time() < end_time:
        alarms_tab = table_parser.table(
            cli.fm(alarmcmd, ssh_client=con_ssh, use_telnet=use_telnet,
...
def wait_for_alarms_gone(alarms, timeout=120, check_interval=3,
...
def get_system_values(fields='name', rtn_dict=False,
                      ...
                      use_telnet=False, con_telnet=None):
    table_ = table_parser.table(
        cli.system('show', ssh_client=con_ssh,
                   use_telnet=use_telnet, con_telnet=con_telnet,
                   auth_info=auth_info)[1])
...
def get_dns_servers(auth_info=Tenant.get('admin_platform'), con_ssh=None,
    ...
    table_ = table_parser.table(cli.system('dns-show', ssh_client=con_ssh,
                                           auth_info=auth_info,
                                           con_telnet=con_telnet,
                                           use_telnet=use_telnet)[1])
    dns_servers = table_parser.get_value_two_col_table(
        table_, 'nameservers').strip().split(sep=',')
...
def set_dns_servers(nameservers, with_action_option=None, check_first=True,
    ...
    if post_dns_servers != nameservers:
        raise exceptions.SysinvError('dns servers expected: {}; actual: {}'.
                                     format(nameservers, post_dns_servers))
...
def generate_event(event_id='300.005', state='set', severity='critical',
                   ...
                   entity_id='CGCSAuto', unknown_text='unknown1',
...
def get_build_info(con_ssh=None, use_telnet=False, con_telnet=None,
                   refresh=False):
    """
...
def get_sw_version(con_ssh=None, use_telnet=False, con_telnet=None,
                   use_existing=True):
    """
    ...
    info_dict = get_build_info(con_ssh=con_ssh, use_telnet=use_telnet,
                               con_telnet=con_telnet, refresh=True)
...
def get_snmp_comms(field='SNMP community', con_ssh=None,
    ...
    """
    table_ = table_parser.table(cli.system('snmp-comm-list',
                                           ssh_client=con_ssh,
                                           auth_info=auth_info)[1])
...
def modify_oam_ips(fail_ok=False, con_ssh=None,
    ...
    if not standby_configured:
        ...
        raise exceptions.SysinvError(
            ...
            "lock/unlock failed after oam-modify.")
...
def get_controller_uptime(con_ssh, auth_info=Tenant.get('admin_platform')):
    ...
    if standby_con:
        standby_con_uptime = int(
            get_host_values(host=standby_con, fields='uptime',
    ...
    for field in args_dict:
        ...
        if actual_val != expt_val:
            raise exceptions.SysinvError(
                ...
                "actual: {}".format(field, expt_val, actual_val))
...
def get_data_networks(field='name', con_ssh=None,
    ...
    """
    table_ = table_parser.table(cli.system('datanetwork-list',
                                           ssh_client=con_ssh,
                                           auth_info=auth_info)[1])
...
def wait_for_hosts_states(hosts, timeout=HostTimeout.REBOOT, check_interval=5,
                          ...
                          use_telnet=False, con_telnet=None, fail_ok=True,
                          auth_info=Tenant.get('admin_platform'),
                          **states):
    """
    ...
    end_time = time.time() + timeout
    while time.time() < end_time:
        if __hosts_stay_in_states(hosts, con_ssh=con_ssh, use_telnet=use_telnet,
                                  con_telnet=con_telnet,
...
def __hosts_in_states(hosts, con_ssh=None, use_telnet=False, con_telnet=None,
                      auth_info=Tenant.get('admin_platform'),
                      **states):
...
def is_active_controller(host, con_ssh=None, use_telnet=False, con_telnet=None,
    ...
    personality = get_host_values(host, fields='capabilities',
                                  auth_info=auth_info,
                                  merge_lines=True, con_ssh=con_ssh,
                                  use_telnet=use_telnet,
                                  con_telnet=con_telnet)[0].get(
        'Personality', '')
