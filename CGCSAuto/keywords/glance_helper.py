...
def create_image(name=None, image_id=None, source_image_file=None, volume=None,
    ...
    if file_path:
        if file_path.startswith('~/'):
            file_path.replace('~', HostLinuxUser.get_home(), 1)
...
def wait_for_image_sync_on_subcloud(image_id, timeout=1000, delete=False):
    if ProjVar.get_var('IS_DC'):
        if dc_helper.get_subclouds(
            ...
            if delete:
                _wait_for_images_deleted(images=image_id, auth_info=auth_info,
...
def delete_images(images, timeout=ImageTimeout.DELETE, check_first=True,
                  ...
                  sys_con_for_dc=True, wait_for_subcloud_sync=True,
    ...
    if ProjVar.get_var('IS_DC') and wait_for_subcloud_sync:
        wait_for_image_sync_on_subcloud(images_deleted, timeout=1000,
                                        delete=True)
        if del_subcloud_cache:
            LOG.info("Attempt to delete glance image cache on subclouds.")
            # glance image cache on subcloud expires only after 24 hours of
            # glance image-delete. So it will fill up the
            # /opt/cgcs file system quickly in automated tests. Workaround
            # added to manually delete the glance cache.
            subclouds = dc_helper.get_subclouds(field='name', avail='online',
                                                mgmt='managed')
            for subcloud in subclouds:
                subcoud_ssh = ControllerClient.get_active_controller(
                    name=subcloud, fail_ok=True)
                if subcoud_ssh:
                    for img in images_deleted:
                        img_path = '/opt/cgcs/glance/image-cache/{}'.format(img)
                        if subcoud_ssh.file_exists(img_path):
                            subcoud_ssh.exec_sudo_cmd('rm -f {}'.format(
                                img_path))
...
def get_guest_image(guest_os, rm_image=True, check_disk=False, cleanup=None,
    ...
    Args:
        ...
        rm_image (bool): whether or not to rm image from image dir
