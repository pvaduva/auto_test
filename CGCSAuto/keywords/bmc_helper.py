...
def suppress_sensor(sensorgroup_name, host):
    ...
    return _suppress_unsuppress_sensor(sensorgroup_name, host,
                                       set_suppress='True')
...
def suppress_sensorgroup(sensorgroup_name, host):
    ...
    return _suppress_unsuppress_sensor(sensorgroup_name, host,
                                       set_suppress='True', sensor_group=True)
...
def unsuppress_sensorgroup(sensor_name, host):
    ...
    return _suppress_unsuppress_sensor(sensor_name, host,
                                       set_suppress='False', sensor_group=True)


def _suppress_unsuppress_sensor(sensor_name, host, set_suppress='False',
                                sensor_group=False):
    ...
    if sensor_suppression_value != set_suppress:
        ...
        res, out = cli.system(sysinv_action, '{} {} suppress={}'.format(
            host, sensor_uuid, set_suppress), fail_ok=True)
...
def set_sensorgroup_audit_interval(sensorgroup_name, host, audit_value=10):
    ...
    return set_sensor_audit_interval(sensorgroup_name, host, audit_value,
                                     sensor_group=True)


def set_sensor_audit_interval(sensor_name, host, audit_value=10,
                              sensor_group=False):
    """main suppress/unsuppress routine."""
    ...
    res, out = cli.system(sysinv_action, '{} {} {}={}'.format(
        host, sensor_uuid, audit_action, audit_value),
                          fail_ok=True)
...
def set_sensorgroup_action(sensorgroup_name, host,
                           event_level='actions_critical', action='ignore'):
    """Modify the sensor action."""

    return set_sensor_action(sensorgroup_name, host, event_level, action,
                             sensor_group=True)


def modify_sensorgroup(host, sensor_group, value='name', action_critical=None,
                       action_major=None, action_minor=None,
    ...
    if not args:
        raise ValueError("At least one field should be specified: {}".format(
            args_dict.keys()))

    if value == 'name':
        sensor_group_uuid = get_sensor_uuid(sensor_group, host,
                                            sensor_group=True)
    else:
    ...
    code, out = cli.system('host-sensorgroup-modify', args, ssh_client=con_ssh,
    ...
    for key, val in validate_dict.items():
        post_val = table_parser.get_value_two_col_table(post_mod_tab,
                                                        field=key)
        if not str(val) == post_val:
            failed_to_mod += '\n{} val is {} instead of {} after modify'.format(
                key, post_val, val)

    if failed_to_mod:
        raise exceptions.SysinvError(
            "Failed to modify sensorgroup to specified value. {}".format(
                failed_to_mod))

    LOG.info("{} sensorgroup {} successfully modified to: {}".format(
        host, sensor_group, validate_dict))
    return 0, "{} sensorgroup {} successfully modified".format(host,
                                                               sensor_group)


def set_sensor_action(sensor_name, host, event_level='actions_critical',
                      action='ignore', audit_interval=None,
                      sensor_group=False):
    """Modify the sensor action."""
    ...
    sensor_action = get_sensors_action(sensor_uuid, host, event_level,
                                       sensor_group)
    ...
    res, out = cli.system(sysinv_action, '{} {} {}'.format(
        host, sensor_uuid, args.strip()), fail_ok=True)

    post_sensor_action = get_sensors_action(sensor_uuid, host, event_level,
                                            sensor_group)

    LOG.info("Set sensor action res: {}\n {} setting: {}".format(
        res, sensor_name, post_sensor_action))
    return res == 0 and post_sensor_action == action


def get_sensors_action(sensor_uuid, host, event_level='actions_critical',
                       sensor_group=False):
    """
    ...
    sensor_showtable = get_sensor_showtable(sensor_uuid, host, sensor_group)
    sensor_action = table_parser.get_value_two_col_table(sensor_showtable,
                                                         event_level)
...
def get_sensors_table(host=None, sensor_group=False):
    ...
    res, out = cli.system('{}'.format(sysinv_action),
                          '{} --nowrap'.format(host), fail_ok=True)
...
def get_sensor_uuid(sensor_name, host, sensor_group=False):
    ...
    sensor_uuid = table_parser.get_values(sensor_table, 'uuid',
                                          Name=sensor_name)[0]
...
def get_first_sensor_from_sensorgroup(sensor_groupname, host):
    ...
    sensors = ast.literal_eval(table_parser.get_values(
        sensorgroup_table, 'sensors', name=sensor_groupname)[0])
    return sensors[0]
...
def get_sensor_alarm(host, sensor_name):
    ...
    alarms_table = system_helper.get_alarms_table(
        query_key='entity_instance_id', query_value=query_value,
        query_type='string')
...
def check_host_state(host, expected_host_state):
    ...
    return system_helper.wait_for_hosts_states(
        host, timeout=90, check_interval=10, con_ssh=con_ssh,
...
def trigger_event(host, sensor_name, sensor_value):
    ...
    LOG.info("Update the sensordata file /var/run/ipmitool/{} to trigger "
             "the sensor: {}".format(sensor_data_file_name, sensor_name))
    ...
    expression = r"sed 's/\({}.* | .* | .* |\) ok/\1 {}/'".format(sensor_name,
                                                                  sensor_value)

    con_ssh.exec_sudo_cmd(cmd='{} < {} > {}'.format(
        expression, sensor_data_file, tmp_sensor_datafile), fail_ok=False)

    con_ssh.exec_sudo_cmd(cmd='cp {} {}'.format(
        tmp_sensor_datafile, sensor_data_file), fail_ok=False)

    LOG.info("Check sed sensor status successful")
    output = con_ssh.exec_sudo_cmd(cmd='grep "{}" {}'.format(
        sensor_name, sensor_data_file), fail_ok=False)[1]

    escaped_name = re.escape(sensor_name)
    assert re.search(r'{} .* \| {}'.format(escaped_name, sensor_value),
                     output), "sed unsuccessful"
...
def clear_events(host):
    ...
    con_ssh.exec_sudo_cmd(cmd='cp {} {}'.format(
        original_sensor_datafile, sensor_data_file), fail_ok=False)
...
def backup_sensor_data_files(hosts=None, con_ssh=None):
    ...
    con_ssh = ControllerClient.get_active_controller() if not con_ssh else \
        con_ssh
    for host in hosts:
        dest_path = "{}/hwmond_{}_sensor_data".format(
            HostLinuxUser.get_home(), host)
        if con_ssh.file_exists(dest_path):
            ...
            if con_ssh.file_exists(source_path):
                con_ssh.exec_sudo_cmd('cp {} {}'.format(
                    source_path, dest_path), fail_ok=False)
                hosts_with_file.append(host)
