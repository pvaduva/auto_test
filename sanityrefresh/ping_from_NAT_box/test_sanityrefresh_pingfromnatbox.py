...
e.g. ./test_sanityrefresh_pingfromnatbox.py 10.10.10.2 
...
3) For each VM, extract the public IP
4) Connect to the NAT box, and attempt to ping each VM 
...
def test_sanityrefresh_pingfromnatbox(conn):
    ...
    logging.info("Test Step 4: Extract VM management IPs and ping from the NAT box")    
    result = vm.ping_vms_from_natbox(conn, ping_duration=15)
    if result == True:
        testFailed_flag = True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        sys.exit("Usage: ./test_sanityrefresh_pingfromnatbox.py <Floating IP of host machine>")
    else:
